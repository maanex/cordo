...
export default class PermissionChecks {
    static isBotOwner(userid) {
        if (!Cordo._data.config.botAdmins)
            return false;
    ...
    static async componentPermissionCheck(i) {
        ...
        if (i.data.flags.includes(InteractionComponentFlag.ACCESS_BOT_ADMIN)) {
            UserErrorMessages.interactionNotPermitted(i, Cordo._data.config.texts.interaction_not_permitted_description_bot_admin);
            return false;
        }
        if (!i.data.flags.includes(InteractionComponentFlag.ACCESS_EVERYONE)) {
            ...
            if (interactionOwner?.id !== i.user.id) {
                UserErrorMessages.interactionNotOwned(i, i.message.interaction ? `/${i.message.interaction?.name}` : 'the command yourself', interactionOwner?.username || 'the interaction owner');
                return false;
            }
        }
        if (!i.member)
        ...
        if (i.data.flags.includes(InteractionComponentFlag.ACCESS_ADMIN) && !PermissionStrings.containsAdmin(i.member.permissions)) {
            UserErrorMessages.interactionNotPermitted(i, Cordo._data.config.texts.interaction_not_permitted_description_guild_admin);
            return false;
        }
        if (i.data.flags.includes(InteractionComponentFlag.ACCESS_MANAGE_SERVER) && !PermissionStrings.containsManageServer(i.member.permissions)) {
            UserErrorMessages.interactionNotPermitted(i, Cordo._data.config.texts.interaction_not_permitted_description_manage_server);
            return false;
        }
        if (i.data.flags.includes(InteractionComponentFlag.ACCESS_MANAGE_MESSAGES) && !PermissionStrings.containsManageMessages(i.member.permissions)) {
            UserErrorMessages.interactionNotPermitted(i, Cordo._data.config.texts.interaction_not_permitted_description_manage_messages);
            return false;
        }
        return true;
    }
