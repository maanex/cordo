...
export default class PermissionStrings {
  ...
  public static permissionStringContains(string: string | bigint, per: number | bigint | (keyof typeof PermissionStrings.BIT)) {
    if (typeof string !== 'bigint')
      string = BigInt(string)
    if (typeof per !== 'bigint')
      per = PermissionStrings.BIT[per] ?? BigInt(per)
    return (string & per as bigint) !== 0n
  }
