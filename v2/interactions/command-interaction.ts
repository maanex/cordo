...
export function parseCommandInteractionData(data: RawCommandInteractionData): CommandInteractionData {
  let target: any = undefined
  if (data.type === Const.InteractionCommandType.MESSAGE)
    target = data.resolved?.messages?.[data.target_id]
  else if (data.type === Const.InteractionCommandType.USER)
  ...
  return {
    ...
    targetId: data.target_id as any,
    target,
    options
  }
}
